...
send_packet(void *ptr)
{
  ...
  if(nbr_table_head(ds6_neighbors) != NULL){
    for(uint i = 0; i < len_local_neighborg; i++) {
      ...
      if( (i +1) != NODEID){
        ...
        */
        ...
        write_byte((uint8_t) ':');
        write_byte(local_neighborg_addr[i]->u8[15]);
        write_byte(local_neighborg_addr[i]->u8[14]);
        ...
        printf("S:0X%02X%02X:%d:", 
          (*local_neighborg_addr[i]).u8[14],
          (*local_neighborg_addr[i]).u8[15], seq_id);
        int64_t value = 0;
        /* asn */
        ...
        // uip_udp_packet_sendto(client_conn, buf, BUF_LEN, local_neighborg_addr[i], UIP_HTONS(UDP_PORT));
    ...
    uip_ipaddr_copy(&client_conn->ripaddr, local_neighborg_addr[i]);
    ...
    tcpip_output((const uip_lladdr_t *) mac_neighborg_addr[i]);
